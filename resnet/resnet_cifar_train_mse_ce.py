...
# -*- encoding: utf-8 -*-
"""
@File    :   resnet_cifar_train_mse_ce.py
...
run this file with
"python resnet_cifar_train_mse_ce.py --num_blocks=3 --data_type=cifar_10"
"""
import numpy as np
...
PAR_PATH = "./" + _DATA_TYPE + "/" + str(6 * _NUM_BLOCKS + 2) + "/" + _loss_style
...
def train(itera, f):
    ...
    print("parameter settings, num_layers: %d, data_type: %s, loss_style: %s"
          % (6 * FLAGS.num_blocks + 2, _DATA_TYPE, _loss_style))
    print("parameter settings, num_layers: %d, data_type: %s, loss_style: %s"
          % (6 * FLAGS.num_blocks + 2, _DATA_TYPE, _loss_style), file=f)
    ...
    for i in range(itera):
        ...
        i_global, _, l_loss, l_ce_loss, l_mse_loss, l_acc = \
            sess.run([global_step, optimizer, loss, ce_loss, mse_loss, accuracy],
                     feed_dict={x: batch_xs, y: batch_ys, phase_train: True})

        if i_global % 10 == 0:
            print("Global step: %d, batch accuracy: %.4f, loss: %.4f, ce loss: %.4f, mse_loss: %.4f"
                  % (i_global, l_acc, l_loss, l_ce_loss, l_mse_loss))
            print("Global step: %d, batch accuracy: %.4f, loss: %.4f, ce loss: %.4f, mse_loss: %.4f"
                  % (i_global, l_acc, l_loss, l_ce_loss, l_mse_loss), file=f)
    ...
    return _ce_loss, _mse_loss, _test_acc, _cm, _feature
...
def main(_):
    ...
    ce_loss, mse_loss, test_acc, cm, feature = train(_ITERATION, f)
    ...
    np.savetxt(os.path.join(PAR_PATH, "mse_loss.txt"), mse_loss)
